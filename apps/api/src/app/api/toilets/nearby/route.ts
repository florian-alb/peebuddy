...
export async function GET(request: NextRequest) {
  try {
    ...
    const radiusKm = searchParams.get("radius") ? parseFloat(searchParams.get("radius")!) : 5; // Default 5km
    const limit = searchParams.get("limit") ? parseInt(searchParams.get("limit")!) : 10;
    
    // Parse filter parameters
    const isFree = searchParams.get("is_free") === "true" ? true : 
                  searchParams.get("is_free") === "false" ? false : undefined;
    const isPublic = searchParams.get("is_public") === "true" ? true : 
                    searchParams.get("is_public") === "false" ? false : undefined;
    const isHandicap = searchParams.get("is_handicap") === "true" ? true : 
                      searchParams.get("is_handicap") === "false" ? false : undefined;
    const isCommerce = searchParams.get("is_commerce") === "true" ? true : 
                      searchParams.get("is_commerce") === "false" ? false : undefined;
